...
with tf.Graph().as_default():
    ...
    for ep in xrange(epochs):
        ...
        if ep % 100 == 0:
            ...
            image.imsave('test_img_{}_1.png'.format(ep), testOut)
            testOutDr = test_dr_out[0,:,:,0]
            image.imsave('test_img_drout{}_1.png'.format(ep), testOutDr)
            testOutRN = test_res_out[0,:,:,0]
            image.imsave('test_img_resn{}_1.png'.format(ep), testOutRN)
            testOutunet = test_unet_out[0,:,:,0]
            image.imsave('test_img_unet{}_1.png'.format(ep), testOutunet)
